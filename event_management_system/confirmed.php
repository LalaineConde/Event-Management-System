<?php
    ...
    $display_confirmed = "SELECT * FROM events_details WHERE status = 'CONFIRMED'";
    $query = mysqli_query($connection, $display_confirmed);

    if (isset($_GET['event_id'])) {
        $confirmed_event_id = $_GET['event_id'];

        $update_status_query = "UPDATE events_details SET status = 'CONFIRMED' WHERE event_id = ?";

        $stmt = mysqli_prepare($connection, $update_status_query);
        mysqli_stmt_bind_param($stmt, 'i', $confirmed_event_id);
    ...
    <section class="container1">
        <div class="pending_container" style="border: 3px solid gray; background: <?php echo $backgroundColor; ?>">
            <div class="left-content">
                ...
                <br>
                <br>
                <a href="edit-event.php?event_id=<?php echo $event_id; ?>&status=CONFIRMED">EDIT</a>
                <a href="delete-event.php?event_id=<?php echo $event_id; ?>&status=CONFIRMED">DELETE</a>
            </div>
            ...
            <div class="right-content">
                ...
                <a href="completed.php?event_id=<?php echo $event_id; ?>">COMPLETE</a>
                </div>
            </div>
        </div>
    </section>
